    ...
    */
    ...
    var contentGenerator = function(d) {
        ...
        trowEnter.append("td")
            ...
            .html(function(p, i) {
              // add
              return valueFormatter(Math.abs(p.value), i);
              // add end
            });
...
　　　　　    //グラフ範囲(横: 最大値にあわせて原点をcenterに合わせる)
            var dy;
            if (tuna) {
              dy = d3.max( d3.merge(seriesData).map(function(d) { return Math.abs(d.y) }))
              y.domain([-dy, dy])
            } else {
